...
def create_dns_record(api, config, fqdn, ip_address, ttl, rtype):
    ...
    print(f"Creating DNS record: {fqdn} -> {ip_address} {ttl} [{rtype}]")
    #Send BODY information via PUT
    resp = api.put (url_path, data=json.dumps(vs_data))
...
def create_dnsrecord(spec, **kwargs):
    ...
    print('New AVIDnsRecord object detected with following config: ')
    print(yaml.dump(new_dns_record, default_flow_style=False))
